...
public class RegisterFormController extends SimpleFormController {
	...
	protected Object formBackingObject(HttpServletRequest request) throws Exception {
		...
		if (userSession != null) {
			return new AccountForm(this.topic.getAccount(userSession.getAccount().getUsername()));
		}
		else {
	...
	protected Map referenceData(HttpServletRequest request) throws Exception {
		...
		model.put("categories", this.topic.getCategoryList());
		return model;
	}
	...
	protected ModelAndView onSubmit(
		...
		try {
			if (accountForm.isNewAccount()) {
				this.topic.insertAccount(accountForm.getAccount());
			}
			else {
				this.topic.updateAccount(accountForm.getAccount());
			}
		}
		...
		UserSession userSession = new UserSession(this.topic.getAccount(accountForm.getAccount().getUsername()));
		PagedListHolder myList = new PagedListHolder(
				this.topic.getProductListByCategory(accountForm.getAccount().getFavouriteCategoryId()));
